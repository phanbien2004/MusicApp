...
export default function PlaylistDetailScreen() {
    ...
    const loadDetail = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const [storedUserId, storedPlaylistId] = await Promise.all([
    ...
    // --- SEARCH EFFECTS ---
    useEffect(() => {
        if (!searchKeyword.trim()) return setSearchResults([]);
    ...
    useEffect(() => {
        if (!inviteKeyword.trim()) return setInviteResults([]);
        const delayDebounce = setTimeout(async () => {
            setIsSearching(true);
            try {
                const res = await searchAPI({ keyword: inviteKeyword, type: 'members', pageNumber: 1, pageSize: 20 });
                const accepted = res.memberPreviewDTOS?.content.filter((u: any) => u.friendStatus === 'ACCEPTED') || [];
                setInviteResults(accepted);
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                    ...
                    <View style={styles.headerContainer}>
                        <View style={styles.coverWrapper}>
                            {detail.thumbnailUrl ? 
                                ...
                                : <Image source={IMAGE.defaultThumbnail} style={styles.coverImg} />}
                        </View>
                        ...
                        <View style={styles.actionRow}>
                            <View style={styles.leftActions}>
                                {canEdit ? (
                                    ...
                                    <View style={styles.viewOnlyBadge}><Text style={styles.viewOnlyText}>PUBLIC PLAYLIST</Text></View>
                                )}
                            </View>
                            
                            {/* NÚT PLAY CHÍNH ĐÃ CẬP NHẬT LOGIC */}
                            <View style={styles.rightActions}>
                                <Ionicons name="shuffle" size={28} color="#FFF" style={{ marginRight: 15 }} />
                                <TouchableOpacity style={styles.playBtnLarge} onPress={handleTogglePlayPlaylist}>
                                    <Ionicons 
                                        name={status.playing && activePlaylistId === detail.id.toString() ? "pause" : "play"} 
                                        size={28} color="#000" 
                                    />
                                </TouchableOpacity>
                            </View>
                ...
                renderItem={({ item }) => {
                    // Logic lọc: Chỉ highlight khi bài đó đang phát TRONG playlist này
                    const isTrackPlayingHere = currentTrack?.id === item.id && activePlaylistId === detail.id.toString();

                    return (
                        <TouchableOpacity 
                            style={[styles.trackItem, isTrackPlayingHere && styles.trackItemActive]} 
                            onPress={() => handleTrackSelect(item)}
                        >
                            <Image source={{ uri: item.thumbnailUrl }} style={styles.trackThumb} />
                            <View style={{ flex: 1 }}>
                                <Text style={[styles.trackTitle, isTrackPlayingHere && { color: Colors.teal }]} numberOfLines={1}>{item.title}</Text>
                                <Text style={styles.trackArtist}>{(item as any).contributors?.[0]?.name || 'Unknown'}</Text>
                            </View>

                            {/* HIỆU ỨNG SÓNG NHẠC ĐỘNG */}
                            {isTrackPlayingHere && status.playing ? (
                                <PlayingAnimation />
                            ) : isTrackPlayingHere ? (
                                <Ionicons name="play" size={16} color={Colors.teal} style={{ marginRight: 8 }} />
                            ) : null}
                ...
                <View style={styles.modalOverlay}>
                    ...
                    <View style={[styles.modalSheet, { paddingBottom: insets.bottom + 20 }]}>
                        ...
                        <View style={styles.modalTopBar}>
                            ...
                            <Text style={styles.modalMainTitle}>Edit Playlist</Text>
                            <TouchableOpacity onPress={handleSaveEdit} disabled={isUpdating}>
                                {isUpdating ? <ActivityIndicator size="small" color={Colors.teal} /> : <Text style={styles.modalSaveText}>Save</Text>}
                            </TouchableOpacity>
                        </View>
                        <View style={styles.modalForm}>
                            <TouchableOpacity style={styles.modalImageSquare} onPress={async () => {
                            ...
                            <View style={{ flex: 1, justifyContent: 'center' }}>
                                <TextInput style={styles.modalInput} value={editName} onChangeText={setEditName} placeholder="Name" placeholderTextColor="#000" />
                                <Text style={styles.inputLabelHint}>EDIT NAME</Text>
                            </View>
                        </View>
                        <TextInput style={styles.descInput} value={editDesc} onChangeText={setEditDesc} placeholder="Description..." placeholderTextColor="#AAA" multiline />
                        <TouchableOpacity style={styles.modalActionItem} onPress={() => setIsPublic(!isPublic)}>
                            <Ionicons name={isPublic ? "lock-open-outline" : "lock-closed-outline"} size={22} color="#FFF" />
                            <Text style={styles.modalActionText}>{isPublic ? 'SET TO PRIVATE' : 'SET TO PUBLIC'}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </Modal>
                ...
                <View style={styles.fullModal}>
                    <View style={[styles.searchBarHeader, { paddingTop: insets.top + 10 }]}>
                        ...
                        <View style={styles.searchInner}>
                            <TextInput style={styles.searchInput} placeholder="Search tracks..." placeholderTextColor="#888" value={searchKeyword} onChangeText={setSearchKeyword} autoFocus />
                        </View>
                    </View>
                    <FlatList data={searchResults} keyExtractor={(item) => `search-${item.id}`} renderItem={({ item }) => (
                        <View style={styles.searchItem}>
                            ...
                            <View style={{ flex: 1, marginLeft: 12 }}><Text style={styles.searchTitle}>{item.title}</Text></View>
                            <TouchableOpacity onPress={() => handleAddTrack(item.id)}><Ionicons name="add-circle" size={30} color={Colors.teal} /></TouchableOpacity>
                        </View>
                    ...
                    <View style={styles.trackOptionsSheet}>
                        ...
                        {selectedTrack && (
                            <View style={styles.trackInfoPreview}>
                                ...
                                <View><Text style={styles.previewTitle}>{selectedTrack.title}</Text><Text style={styles.previewSub}>Remove this track</Text></View>
                            </View>
                        )}
...
const styles = StyleSheet.create({
    ...
    coverWrapper: { width: width * 0.55, height: width * 0.55, borderRadius: 20, backgroundColor: '#222', alignSelf: 'center', marginTop: 40, overflow: 'hidden' },
    ...
    viewOnlyText: { color: Colors.teal, fontSize: 10, fontWeight: '800' },
    ...
    // TRACK ITEM
    trackItem: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 8, paddingHorizontal: 10, borderRadius: 12, marginBottom: 4 },
    trackItemActive: { backgroundColor: 'rgba(20, 184, 166, 0.15)' },
    ...
    // EQUALIZER
    eqContainer: { flexDirection: 'row', alignItems: 'flex-end', gap: 2, height: 20, marginRight: 8 },
    ...
    modalInput: { backgroundColor: '#777', height: 42, borderRadius: 8, paddingHorizontal: 12, color: '#000', fontWeight: '700' },
    inputLabelHint: { color: '#888', fontSize: 11, marginTop: 5 },
    ...
    // INVITE MODAL
    inviteSheet: { backgroundColor: '#121212', borderTopLeftRadius: 30, borderTopRightRadius: 30, paddingHorizontal: 20 },
    collabList: { marginVertical: 15, gap: 15 },
    userRowItem: { flexDirection: 'row', alignItems: 'center', gap: 12 },
    userAvatarPlaceholder: { width: 40, height: 40, borderRadius: 20 },
    userNameText: { color: '#FFF', fontSize: 15, fontWeight: '700' },
    userRoleLabel: { color: '#888', fontSize: 12 },
    horizontalDivider: { height: 1, backgroundColor: '#222', marginVertical: 10 },
    inviteHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
    inviteSectionTitle: { color: '#FFF', fontSize: 16, fontWeight: '800' },
    searchBoxSmall: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 8, paddingHorizontal: 10, height: 34, marginLeft: 15 },
    searchBoxInput: { flex: 1, color: '#FFF', fontSize: 12 },
    inviteFriendCard: { backgroundColor: '#EAEAEA', width: 110, borderRadius: 15, padding: 10, alignItems: 'center', marginRight: 10 },
    friendAvatarCircle: { width: 36, height: 36, borderRadius: 18, marginBottom: 5 },
    ...
    searchInner: { flex: 1, height: 40, backgroundColor: '#1A1A1A', borderRadius: 8, paddingHorizontal: 10, justifyContent: 'center' },
    ...
    trackInfoPreview: { flexDirection: 'row', alignItems: 'center', gap: 15, marginBottom: 20 },
